...
```text
本页设备数 + 页号 + 总页数 + 设备总数
+ N × (Node 类型 + Node ID LE16 + RSSI 绝对值 + 软件版本)
```

- 分页头固定4字节；页号从0开始，总页数范围为1～6。
- 单页最多50项，设备总数使用一字节表示，完整列表最多255项。
- 工具按页号去重并等待全部页；各页 Async 地址、总页数和设备总数必须一致。
- Node ID 必须非零且在完整列表中不重复，类型范围为2～63。
- RSSI 绝对值范围为0～200；`0` 表示未上线。界面同时显示在线与离线 Node，并按在线在前、离线在后排列；离线 Node 禁止加入升级目标。
...
- 收齐全部分页、超时或取消后立即注销处理器；相同页的重复响应不会覆盖结果，内容冲突时明确报错。
...
1. Async 的 `0x0F` 必须返回4字节分页头和规定的5字节设备项，并输出 RSSI 绝对值。
